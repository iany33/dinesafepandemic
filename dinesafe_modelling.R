...
pred2 <- predictions(m3, type = "response", 
                     newdata = datagrid(model = m3, pandemic = "Pandemic Period", 
                                        time = seq(169, 280, by = 2), inspections = 1)) |> posteriordraws()
...
mfx <- marginaleffects(m3, type = "response", variables = "pandemic", 
                       newdata = datagrid(time = seq(1, 280, by = 2), inspections = 1)) |> 
  posteriordraws()
...
cowplot::plot_grid(p1, p2, labels = c("A", "B"), nrow=1, ncol=2)
...
mfx <- marginaleffects(m3, type = "response", variables = "pandemic", 
                       newdata = datagrid(time = c(mean(169:261), mean(262:280)), 
...
marginaleffects(m3, type = "response", variables = "pandemic", 
                newdata = datagrid(time = seq(1, 280, by = 2), inspections = 1)) |> summary()

marginaleffects(m3, type = "response", variables = "pandemic", 
                newdata = datagrid(time = seq(1, 261, by = 2), inspections = 1)) |> summary()

marginaleffects(m3, type = "response", variables = "pandemic", 
                newdata = datagrid(time = seq(262, 280, by = 2), inspections = 1)) |> summary()
...
marginaleffects(r3, type = "response", variables = "pandemic",
                        newdata = datagrid(time = seq(1, 280, by = 2), 
